 ...
 */
class Customer_care_staff extends MY_Controller {
    ...
    public function index($offset = 0) {
        ...
        $get = $this->input->get();
        $conditional = array();
        $conditional['where'] = array('role_id' => 10);
         ...
         */
        $data['pagination'] = $this->_create_pagination_link('MANAGERS/customer_care_staff/index', $data_pagination['total_row'], 4);
        $data['customer_care'] = $data_pagination['data'];
        $data['total_customer_care'] = $data_pagination['total_row'];

        $data['content'] = 'MANAGERS/customer_care_staff/index';
        $tableArr = 'username name phone active action';
        $data['load_js'] = array(
    ...
    function action_edit_teacher($id) {
        if ($id == 0) {
            ...
            $param = array();
            $editArr = array('username', 'name', 'email', 'phone', 'course', 'active', 'vat1','vat2','vat3', 'cod', 'profit1', 'profit2', 'profit3');
            ...
            for ($i = 1; $i <= 3; $i++) {
                $param['vat'][] = $param['vat' . $i];
                unset($param['vat' . $i]);
                $param['profit'][] = $param['profit' . $i];
                unset($param['profit' . $i]);
            }

            $param['vat'] = implode(';', $param['vat']);
            $param['profit'] = implode(';', $param['profit']);
    ...
    function action_add_teacher() {
        ...
        for ($i = 1; $i <= 3; $i++) {
            $post['vat'][] = $post['vat' . $i];
            unset($post['vat' . $i]);
            $post['profit'][] = $post['profit' . $i];
            unset($post['profit' . $i]);
        }

        $post['vat'] = implode(';', $post['vat']);
        $post['profit'] = implode(';', $post['profit']);
